...
export class ExtensionInstance {
    ...
    get draftMessages() {
        const successMessage = this.isDraftable() && !this.isAppConfigExtension
            ? `Draft updated successfully for extension: ${this.localIdentifier}`
            : undefined;
        const errorMessage = this.isDraftable() && !this.isAppConfigExtension ? `Error while deploying updated extension draft` : undefined;
    ...
    async deployConfig({ apiKey, token }) {
        if (this.isFunctionExtension)
            return this.functionDeployConfig({ apiKey, token });
        return this.commonDeployConfig(apiKey);
    }
    async functionDeployConfig({ apiKey, token, }) {
    ...
    async commonDeployConfig(apiKey) {
    ...
    get singleTarget() {
        const targets = getPathValue(this.configuration, 'targeting') ?? [];
        if (targets.length !== 1)
            return undefined;
    ...
    get contextValue() {
        ...
        if (this.isFlow)
            context = this.configuration.handle ?? '';
        return context;
    }
    async bundleConfig({ identifiers, token, apiKey }) {
